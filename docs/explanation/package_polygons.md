...
via a target-schema YAML and produces one dissolved output per level in a
single call. It reuses `core/dissolve/_02_dissolve.py`'s stage function
...
package_polygons("admin3.parquet", target_schema_path="schema.yaml")
```
...
2. **`_02_dissolve`**: loads the target schema, detects every level via
   `detect_levels()`, then calls `core.dissolve`'s `_02_dissolve.main()`
   once per level coarser than the finest, grouping by that level's own
   code column and passing `target_schema` through so every finer-level
   column is dropped unconditionally rather than triggering the
   auto-drop warning. The finest level is never dissolved: its own table
   is `{name}_01` directly, since grouping by its own finest code column
   would be a no-op.
