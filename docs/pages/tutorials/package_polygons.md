...
auto-detected structurally by default (no naming convention assumed), or
via an explicit `--name-field`/`--code-field` pair (the same shape
`schema-map`/`schema-fill` take); no per-level `--group-by` needed.
